...
public class Datum {
	...
	private static String[] arrMonatsname = new String[] { "", "Januar", "Februar", "März", "April", "Mai", "Juni",
			"Juli", "August", "September", "Oktober", "November", "Dezember" };

	private static String[] arrWochentag = new String[] { "Sonntag", "Montag", "Dienstag", "Mittwoch", "Donnerstag",
			"Freitag", "Samstag" };
	...
	public Datum addiereJahre(int anzahl) {
		...
		if (checkJahr(jahr + anzahl)) {
			...
			retValue = false;
			setError("Das Datum liegt außerhalb des Gregorianischen Kalenders.");
		}
	 ...
	 * untenstehende Formel richtige Ergebnisse liefert, muß das Datum zwischen dem
	 * 01.01.1582 und dem 31.12.3000 liegen. Die Berechnung erfolgt mit der
	 * folgenden von Gauß stammenden sogenannten Kalenderformel : Das zu behandelnde
	 * Datum sei in den Variablen tag, monat, jahr abgelegt.
	 *
	 * Damit werden zwei Hilfsgrößen h und k wie folgt berechnet :
	 * 
	 * Ist der Wert von monat <= 2, so ist h = monat + 12 und k = jahr - 1. Ist der
	 * Wert von monat > 2, so ist h = monat und k = jahr. Der Wochentag ergibt sich
	 * nun durch : wochentag = [ tag + 2*h + (3*h + 3) div 5 + k + k div 4 - k div
	 * 100 + k div 400 +1] mod 7
	 * 
	...
	// Formel, die jedoch nur für die Tage vom 1.1.1901 bis zum 31.12.2099 korrekt
	// ist. Da
	// aber in der Zeit zwischen Ostern und Weihnachten keine Schalttage liegen
	// können,
